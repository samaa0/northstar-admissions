...
function InterviewPanel({ data, busy, onUpdated, onError }) {
  const [form, setForm] = useState({ applicationChoiceId: data.choices[0]?.id || '', scheduledDate: '', scheduledTime: '', durationMinutes: '45', mode: 'ONLINE', location: 'Zoom admissions room', panelMemberIds: data.activeStaff.slice(0, 2).map(({ id }) => String(id)), chairId: String(data.activeStaff[0]?.id || '') });
  ...
  async function create(event) {
    ...
    setSaving(true);
    try {
      const { scheduledDate, scheduledTime, ...requestForm } = form;
      const scheduledAt = new Date(`${scheduledDate}T${scheduledTime}`).toISOString();
      await api(`/applications/${data.application.id}/interviews`, { method: 'POST', body: JSON.stringify({ ...requestForm, scheduledAt }) });
      await onUpdated();
  ...
  return <div id="case-panel-interview" className="case-panel interview-panel" role="tabpanel" aria-labelledby="case-tab-interview">
    <section className="case-section"><header className="case-section-heading"><div><CalendarDays size={17} /><div><h3>Schedule interview</h3><p>Panel conflicts and completion rules are enforced by the database.</p></div></div></header>
      <form className="interview-form" onSubmit={create}><label className="field"><span>Choice</span><select value={form.applicationChoiceId} onChange={(event) => setForm((current) => ({ ...current, applicationChoiceId: event.target.value }))}>{data.choices.map((choice) => <option key={choice.id} value={choice.id}>{choice.code} · preference {choice.preference_rank}</option>)}</select></label><label className="field"><span>Scheduled date</span><input type="date" value={form.scheduledDate} onChange={(event) => setForm((current) => ({ ...current, scheduledDate: event.target.value }))} required /></label><label className="field"><span>Scheduled time</span><input type="time" step="300" value={form.scheduledTime} onChange={(event) => setForm((current) => ({ ...current, scheduledTime: event.target.value }))} required /></label><label className="field"><span>Duration</span><input type="number" min="15" max="240" value={form.durationMinutes} onChange={(event) => setForm((current) => ({ ...current, durationMinutes: event.target.value }))} required /></label><label className="field"><span>Mode</span><select value={form.mode} onChange={(event) => setForm((current) => ({ ...current, mode: event.target.value }))}><option value="ONLINE">Online</option><option value="IN_PERSON">In person</option><option value="HYBRID">Hybrid</option></select></label><label className="field full-field"><span>Location</span><input value={form.location} onChange={(event) => setForm((current) => ({ ...current, location: event.target.value }))} required /></label><fieldset className="panel-picker"><legend>Panel members</legend>{data.activeStaff.map((staff) => <label key={staff.id}><input type="checkbox" checked={form.panelMemberIds.includes(String(staff.id))} onChange={() => togglePanel(staff.id)} /><span>{staff.name}</span><select value={String(form.chairId) === String(staff.id) ? 'CHAIR' : 'MEMBER'} onChange={(event) => setForm((current) => ({ ...current, chairId: event.target.value === 'CHAIR' ? String(staff.id) : current.chairId }))}><option value="MEMBER">Member</option><option value="CHAIR">Chair</option></select></label>)}</fieldset><button className="button button-primary" type="submit" disabled={saving || !form.scheduledDate || !form.scheduledTime}><Plus size={15} />Schedule interview</button></form>
    </section>
